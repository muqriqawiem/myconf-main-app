...
export async function handleFileUpload(paperFile: File) {
  ...
  console.log('Writing file to temp directory:', tempFilePath);  //newly added code
  const arrayBuffer = await paperFile.arrayBuffer();
  await fs.writeFile(tempFilePath, Buffer.from(arrayBuffer));  // Write to temp dir

  console.log('File written to temp directory'); //newly added code
  const uploadedFile = await uploadOnCloudinary(tempFilePath);

  console.log('Attempting to delete temp file:', tempFilePath); //newly added code
  await fs.unlink(tempFilePath);  // Clean up temp file
  console.log('Temp file deleted'); //newly added code
