...
const features = [
  {
    title: "Conference Creation & Management",
    description: "Seamlessly create and oversee conferences with customizable settings. Configure submission deadlines, review workflows, and more, all through an intuitive interface designed to streamline the entire process.",
    icon: <FaTools size={40} className="text-blue-600" />,
  },
  {
    title: "Paper Submission System",
    description: "Enhance the paper submission experience for authors with a structured workflow that gathers all necessary information. Our system supports various formats and allows for easy revisions, simplifying the process for both authors and organizers.",
    icon: <FaFileAlt size={40} className="text-green-600" />,
  },
  {
    title: "ConfHub Integration",
    description: "Boost your conference's visibility by integrating with ConfHub, a feature that lists all available conferences and makes them easily discoverable. This ensures broader reach and higher participation, attracting high-quality submissions globally.",
    icon: <FaSearch size={40} className="text-purple-600" />,
  },
  {
    title: "Review Management & Feedback",
    description: "Optimize the review process with tools for easy reviewer assignment, progress tracking, and feedback management. This feature ensures a thorough and organized review process, enhancing the overall quality of submissions.",
    icon: <FaClipboardList size={40} className="text-red-600" />,
  },
  {
    title: "Dynamic Dashboard",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {features.map((feature, index) => (
          <Card key={index} className="bg-gradient-to-br from-white to-gray-50 shadow-lg rounded-xl border border-gray-200 hover:shadow-2xl transition-shadow duration-300 p-6">
            ...
            <CardContent className="py-4">
              <CardDescription className="text-sm text-gray-600 leading-relaxed" style={{ textAlign: 'justify' }}>{feature.description}</CardDescription>
            </CardContent>
            <CardFooter className="flex justify-end pt-4">
